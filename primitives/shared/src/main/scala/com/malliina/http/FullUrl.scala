...
case class FullUrl(proto: String, hostAndPort: String, uri: String):
  ...
  def /(more: String): FullUrl =
    if uri.endsWith("/") then append(more.dropWhile(_ == '/'))
    else append(if more.startsWith("/") then more else s"/$more")
  ...
  def withQuery(qs: (String, String)*): FullUrl =
    val asString = qs
      .map:
        case (k, v) => s"$k=$v"
      .mkString("&")
    val firstChar = if uri.contains("?") then "&" else "?"
...
object FullUrl extends ValidatingCompanion[String, FullUrl]:
  ...
  private def dropHttps(domain: String) =
    val prefix = "https://"
    if domain.startsWith(prefix) then domain.drop(prefix.length) else domain
  ...
  override def build(input: String): Either[ErrorMessage, FullUrl] =
    ...
    if m.find() && m.groupCount() == 3 then Right(FullUrl(m.group(1), m.group(2), m.group(3)))
    else Left(ErrorMessage(s"Invalid URL: '$input'."))
